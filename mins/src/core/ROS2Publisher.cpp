...
void ROS2Publisher::publish_tf() {
  ...
  bf_trans.header.stamp = rclcpp::Clock().now();
  bf_trans.header.frame_id = "imu";
  bf_trans.child_frame_id = "base_footprint";

  // Create quaternions for rotation
  tf2::Quaternion q_rot, q_orig, q_result;
  q_rot.setRPY(0, 3.19 , -0.30); // Rotate: in x , y , z directions

  // Get the original quaternion from `trans`
  q_orig.setX(trans.transform.rotation.x);
  q_orig.setY(trans.transform.rotation.y);
  q_orig.setZ(trans.transform.rotation.z);
  q_orig.setW(trans.transform.rotation.w);

  // Combine the quaternions
  q_result = q_rot * q_orig; // Apply rotation
  q_result.normalize();      // Normalize the result

  // Set the new rotation
  bf_trans.transform.rotation.x = q_result.x();
  bf_trans.transform.rotation.y = q_result.y();
  bf_trans.transform.rotation.z = q_result.z();
  bf_trans.transform.rotation.w = q_result.w();

  // Set translation
  bf_trans.transform.translation.x = 0.0;
  bf_trans.transform.translation.y = 0.0;
  bf_trans.transform.translation.z = 0.0;

  bool publish_global2imu_tf = true; 

  if (publish_global2imu_tf) {
      mTfBr->sendTransform(bf_trans);
  }
...
void ROS2Publisher::publish_cam_features() {
  ...
  sensor_msgs::msg::PointCloud2 cloud = ROS2Helper::ToPointcloud(feats_msckf, "odom");
  pub_cam_msckf->publish(cloud);

  // Get our good SLAM features
  vector<Vector3d> feats_slam = sys->state->get_features_SLAM();
  sensor_msgs::msg::PointCloud2 cloud_SLAM = ROS2Helper::ToPointcloud(feats_slam, "odom");
...
void ROS2Publisher::publish_gps(GPSData gps, bool isGeodetic) {
  ...
  poseGPSinENU.header.frame_id = "odom";
...
void ROS2Publisher::publish_vicon(ViconData data) {
  ...
  poseVicon.header.frame_id = "odom";
...
void ROS2Publisher::publish_lidar_map() {
  ...
  for (const auto &ikd : sys->up_ldr->ikd_data) {
    ...
    map_pointcloud.header.frame_id = "odom";
